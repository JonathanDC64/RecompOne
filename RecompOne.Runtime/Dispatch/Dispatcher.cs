...
public static class Dispatcher
{
    ...
    private static readonly List<IOverlay> _contentOverlays = [];
    private static uint _cmLo = uint.MaxValue, _cmHi;
    ...
    // Identify which signature-carrying overlay is actually resident in the
    // shared region and activate it.
    public static void ResolveByContent()
    {
        var mem = Runtime.Mem;
        if (mem == null) return;
        foreach (var ov in _contentOverlays)
        {
            ...
            lock (_active)
            {
                if (_active.Contains(ov.Name)) return; // already resident
            }
