...
namespace RecompOne.Recompiler;

// The recompiler CLI entry, as a library method so it can be driven both by the
// `recompone` exe (RecompOne.Recompiler.Cli) and in-process by consumers such as
// the King's Field Suite generator (which references this as a library — a
// library has no self-contained-ness, so it works for both self-contained and
// framework-dependent host apps, unlike referencing an exe project).
public static class RecompilerCli
{
